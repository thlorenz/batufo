...
- show off level editor
- explain encoder/decoder of level content
...
- [ ] store current state in local storage so on page refresh (in case things crash) we can get our current level back
- [ ] have a empty + sample button to load canned levels
- [ ] vim/emacs/normal mode selectors
...
- [ ] base64 encoding with 16bits per tile
- [ ] update url query on each /valid/ change
- [ ] load from query string during init
- [ ] script that takes text file, converts it to query + opens in browser to be invoked from emacs or other editor on demand (i.e. via compile command) or on each save
