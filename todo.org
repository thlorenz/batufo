...
** Server Sends inital Server Update

Continue  [[file:client/lib/widgets/game/game_widget.dart::// TODO: see if we can call game.init in _startNewGame][here]]. 
- [ ] for each player request server sends arena matching level
- [ ] once game is full server sends first update with all player locations
...
- [ ] why on earth are we dying immediately?
