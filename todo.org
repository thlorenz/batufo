...
It actually makes sense to increase the bullet originator's score when he hits and kills a player and thus would be more consistent if he also communicates dealt damage.
  ...
  - [ ] figure out whose responsible to deal out damage from a bullet
  - [ ] ensure that we only send updates about our own player even if we /deal/ health damage to another
  - [ ] bullet originator should send a /dealt damage to/ message instead of a full player model of the affected player
...
*** Bullet Killing Shooter Problem
