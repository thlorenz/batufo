...
- [ ] client needs a reliable way to send device via an enum
- [ ] OSX, Android, iOS and Linux use ~Platform~
- [ ] web is assumed when platform query fails
- [ ] client send that enum as part of request info attempt
- [ ] server maintains map of currently active devices and logs that regularly whenever totals are calculated
- [ ] server keeps a life time map aggregating number of games entered by device
...
- [ ] see if a Flutter item can be made unfocusable, or not a Tab target
- [ ] if above does not work try to force the focus on another item when the player enters
- [ ] additionally try to /eat/ the key strokes sent to the game
- [ ] and/or prevent hud from accepting inputs
